...
class Expense private (
  ...
  def amountByParticipant: Money =
    amount.divideBy(participants.length+1).getOrElse(Money.zero)
}
...
object Expense {
  ...
  def create(
    ...
    (
      ...
      participants.traverse(p =>
        Validated.condNec(
          p =!= payer,
          p,
          s"The payer [$payer]  should not be included in the participants"
        )
      )
    ).mapN((ps, _) => new Expense(payer, amount, ps))
  ...
  implicit val showExpense: Show[Expense] =
    Show.show(e =>
      s"Expense[Payer=${e.payer},Amount=${e.amount.show},Participants=${e.participants.mkString_(",")}]"
    )
}
